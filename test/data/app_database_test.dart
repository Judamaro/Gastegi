...
void main() {
  ...
  test('una BD nueva se siembra solo con las categorías', () async {
    ...
    expect(Sqflite.firstIntValue(
        await db.rawQuery('SELECT COUNT(*) FROM expenses')), 0);
  });

  test('la BD queda en la versión 1 con un device_id y las FK activas',
      () async {
  ...
  test('reabrir la BD no vuelve a sembrar ni pierde lo guardado', () async {
    ...
    final beforeIds =
        (await CategoryRepository(reopened).all()).map((c) => c.id).toList();
