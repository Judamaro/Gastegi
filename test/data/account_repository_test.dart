...
void main() {
  ...
  test('alta y edición conservan el saldo que ve el usuario', () async {
    ...
    await accounts.update(id,
        name: 'Cartera', kind: 'Dinero en mano', iconKey: 'wallet', balance: 500);
    final updated = (await accounts.all()).single;
    expect(updated.name, 'Cartera');
    expect(updated.balance, 500);
  });

  test('rechaza nombres duplicados pero permite reutilizar el de una borrada',
      () async {
  ...
  test('la transferencia mueve saldo y no altera el patrimonio', () async {
    ...
    await accounts.transfer(
        fromId: from, toId: to, amount: 100, date: testNow);
  ...
  test('la transferencia es no-op con importe no positivo o misma cuenta',
      () async {
